...
class CDBM_Frontend {
    ...
    private function card($box, $show_description) {
        ...
        if ($box['button_url']) { $text = $box['button_text'] ? $box['button_text'] : __('Read more', 'wp-updates-plugin'); $html .= '<p class="cdbm-box-action"><a class="cdbm-button" href="' . esc_url($box['button_url']) . '">' . esc_html($text) . '</a></p>'; }
